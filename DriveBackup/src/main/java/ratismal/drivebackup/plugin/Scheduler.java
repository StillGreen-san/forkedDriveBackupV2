    ...
    if (config.backupScheduling.enabled) {
      ...
      for (BackupScheduleEntry entry : config.backupScheduling.schedule) {
        ...
        for (int i = 0; i < entry.days.length; i++) {
          ...
          String dayName =
              entry.days[i].getDisplayName(TextStyle.FULL, config.advanced.dateLanguage);
        ...
        MessageUtil.Builder()
            .mmText(
                ...
                "time",
                scheduleMessageTime.format(DateTimeFormatter.ofPattern("hh:mm a")),
                "days",
      ...
      scheduleDriftTask = taskScheduler
          .runTaskTimer(DriveBackup.getInstance(), () -> startBackupThread(), driftInt, driftInt)
          .getTaskId();
      ...
      MessageUtil.Builder()
          .mmText(
              intl("backups-interval-scheduled"),
              "delay",
              String.valueOf(config.backupStorage.delay))
          .toConsole(true)
          .send();
      long interval = SchedulerUtil.sToTicks(config.backupStorage.delay * 60);
      backupTasks.add(taskScheduler
          .runTaskTimerAsynchronously(
              DriveBackup.getInstance(), new UploadThread(), interval, interval)
          .getTaskId());
