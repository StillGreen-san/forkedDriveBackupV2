...
public class HttpLogger implements Interceptor {
    ...
    public @NotNull Response intercept(@NotNull Interceptor.Chain chain) throws IOException {
        ...
        MessageUtil.Builder()
                .text(String.format("Sending request %s", request.url()))
                .toConsole(true)
                .send();
        ...
        MessageUtil.Builder()
                .text(String.format(
                        "Received response for %s in %.1fms", response.request().url(), (t2 - t1) / 1e6d))
                .toConsole(true)
                .send();
        try {
        ...
        if (responseBodyContentType.equals(jsonMediaType)) {
            try {
                ...
                if (responseBodyJson.getString("msg").equals("code_not_authenticated")) {
                    return response
                            .newBuilder()
                            .body(ResponseBody.create(responseBodyString, responseBodyContentType))
                            .build();
                }
                MessageUtil.Builder().text("Resp: " + responseBodyJson).toConsole(true).send();
            } catch (Exception exception) {
                MessageUtil.Builder()
                        .text("Resp: " + responseBodyString)
                        .toConsole(true)
                        .send();
            }
        } else {
