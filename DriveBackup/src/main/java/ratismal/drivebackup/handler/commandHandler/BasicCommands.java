...
public final class BasicCommands {
  ...
  public static void sendDocs(CommandSender player) {
    MessageUtil.Builder()
        .addPrefix(false)
        .mmText(intl("drivebackup-docs-command"), "header", intl("drivebackup-command-header"))
        .to(player)
        .toConsole(false)
        .send();
  }
  ...
  public static void sendVersion(CommandSender player) {
    MessageUtil builder = MessageUtil.Builder()
        ...
        .mmText(
            ...
            "header",
            intl("drivebackup-command-header"),
            "plugin-version",
            DriveBackup.getInstance().getDescription().getVersion(),
            "java-version",
    ...
    builder.to(player).toConsole(false).send();
  }
  ...
  public static void sendHelp(CommandSender player) {
    MessageUtil.Builder()
        .addPrefix(false)
        .mmText(intl("drivebackup-help-command"), "header", intl("drivebackup-command-header"))
        .to(player)
        .toConsole(false)
        .send();
  }
  ...
  public static void sendBriefBackupList(CommandSender player) {
    ...
    if (backupLocations.isEmpty()) {
      ...
      for (int i = 0; i < backupLocations.size(); i++) {
        ...
        if (linkWord != null) {
          locationMessage.append(Component.text(linkWord).color(NamedTextColor.DARK_AQUA));
        }
        locationMessage.append(Component.text(backupLocations.get(i)).color(NamedTextColor.GOLD));
      }
    }
    MessageUtil.Builder()
        .mmText(intl("brief-backup-list"), "list", locationMessage.build())
        .toConsole(false)
        .to(player)
        .send();
    MessageUtil.Builder()
        .mmText(intl("brief-backup-list-help"))
        .toConsole(false)
        .to(player)
        .send();
  }
