...
public class GoogleDriveUploader extends Uploader {
  ...
  public static final String APPLICATION_VND_GOOGLE_APPS_FOLDER =
  ...
  private void retrieveNewAccessToken() throws Exception {
    RequestBody requestBody = new FormBody.Builder()
        ...
        .add(
            "client_secret",
            Obfusticate.decrypt(AuthenticationProvider.GOOGLE_DRIVE.getClientSecret()))
    ...
    service = new Drive.Builder(
            httpTransport,
            JSON_FACTORY,
            setTimeout(new Credential(BearerToken.authorizationHeaderAccessMethod())
  ...
  private static HttpRequestInitializer setTimeout(
  ...
  public void test(java.io.File testFile) {
    try {
      ...
      File uploadedFile =
          service.files().insert(body, testContent).setSupportsAllDrives(true).execute();
  ...
  public void uploadFile(java.io.File file, String type) {
    try {
      ...
      service
          .files()
          .insert(fileMetadata, fileContent)
  ...
  public void setupSharedDrives(CommandSender initiator) throws Exception {
    if (drives != null && !drives.isEmpty()) {
      ...
      logger.log(
          ...
          "1",
          "drive-name",
          intl("default-google-drive-name"));
      int index = 1;
  ...
  private File getFolder(String name) {
    try {
      Drive.Files.List request = service
          .files()
          .list()
          .setQ("mimeType='application/vnd.google-apps.folder' and trashed=false and 'root' in"
              + " parents");
      FileList files = request.execute();
      ;
      for (File folderfiles : files.getItems()) {
  ...
  private void pruneBackups(File folder) throws Exception {
    ...
    if (files.size() > fileLimit) {
      ...
      for (Iterator<ChildReference> iterator = files.iterator(); iterator.hasNext(); ) {
        ...
        Drive.Files.Delete removeItem =
            service.files().delete(file.getId()).setSupportsAllDrives(true);
        removeItem.execute();
        iterator.remove();
      }
    }
