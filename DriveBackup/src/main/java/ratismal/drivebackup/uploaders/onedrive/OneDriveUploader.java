...
public class OneDriveUploader extends Uploader {
  ...
  public void test(java.io.File testFile) {
    try {
      ...
      Request request = new Request.Builder()
          ...
          .url("https://graph.microsoft.com/v1.0/me/drive/root:/" + destination + "/"
  ...
  public void uploadFile(java.io.File file, String type) throws IOException {
    try {
      ...
      Request request = new Request.Builder()
          .addHeader("Authorization", "Bearer " + accessToken)
          .url("https://graph.microsoft.com/v1.0/me/drive/root:/" + folder.getPath() + "/"
      ...
      while (true) {
        ...
        request = new Request.Builder()
            .addHeader(
                ...
                String.format(
                    "bytes %d-%d/%d",
                    getTotalUploaded(),
                    getTotalUploaded() + bytesToUpload.length - 1,
                    file.length()))
            .url(uploadURL)
        ...
        try (Response uploadResponse = DriveBackup.httpClient.newCall(request).execute()) {
          if (uploadResponse.code() == 202) {
            ...
            List<Object> nextExpectedRanges =
                parsedResponse.getJSONArray("nextExpectedRanges").toList();
            ...
            if (retryCount > MAX_RETRY_ATTEMPTS) {
              ...
              throw new IOException(String.format(
                  "Upload failed after %d retries. %d %s",
